...
#### Updates
Note that the loss function is not convex in the tuple \\((P, Q)\\), hence it we will most likely not minimize it but get stuck in a local minimum. We hope that this local minimum is good enough. Updates will be roughly of the following form upon investigating rating \\((i,j,r)\\):
...
Where \\(\\eta\\) corresponds to our learning rate, e.g. fixed at \\(10^{-3}\\) or adapted, typically reduced, with growing number of updates.

I encountered three different approaches towards ordering updates of different variables. I orderer the approaches in increasing performance in my use case.

1. Alternating optimization:

    ```python
    for _ in range(n_epochs): # stop when convergence is reached
      shuffle(dataset)
