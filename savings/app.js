...
function savingsBreak(
  ...
  const costOfLiving = cumulativeSpending(
    hSecond,
    roi,
    annualSpending * durationBreak
  );
  const passiveIncome =
    cumulativeSpending(hSecond + durationBreak, roi, preBreakSavings) -
    preBreakSavings;
  const newAnnualSavings = cumulativeSpending(
    hFirst,
    savingsIncrease,
    annualSavings
  );
  const postBreakSavings = savings(
    hSecond,
    roi,
    savingsIncrease,
    newAnnualSavings
...
function savingsBreakDer(
  ...
  const postBreakSavingsDer = -savings(
    hSecond,
    roi,
    savingsIncrease,
    newAnnualSavings
...
function calculate() {
  ...
  hint = document.getElementById("calculate-hint");
  if (fields.roi == fields.savingsIncrease) {
    hint.hidden = false;
